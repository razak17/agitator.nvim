...
local function git_blame(opts)
    ...
    vim.system(git_args, job_params, vim.schedule_wrap(function(res)
...
local function git_blame_commit_for_line(_opts)
    ...
    if opts.as_of_commit ~= nil then
        git_args = {'git', 'blame', '-L' .. vim.fn.line('.') .. ',' .. vim.fn.line('.'), opts.as_of_commit, '--', opts.fname or relative_fname}
    end
